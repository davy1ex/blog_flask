...
from app.forms import LoginForm, ProfileSettingsForm, TextEditorForm, AddCommentForm
...
def add_post():
    ...
    if form.validate_on_submit():
        ...
        if Validator.data_required(form.text.data):
            ...
            if "png" in file.filename or "jpeg" in file.filename or "jpg" in file.filename:
                ...
                path_save = os.path.join(os.getcwd(), "app", "static", "img", str(post.id))

                if os.path.isdir(path_save):
                    os.system("rm -rf %s" % path_save)

                os.mkdir(os.path.join(os.getcwd(), "app", "static", "img", str(post.id)))
                file.save(os.path.join(path_save, "img.jpeg"))
...
def remove_post(id):
    ...
    os.system("rm -rf %s" % os.path.join("app", "static", "img", id))
